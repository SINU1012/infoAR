...
주소에 접속하면 즉시 AR 세션을 시도하고, 바닥을 인식하면 InFocus 사이트가 홀로그램 패널처럼 떠서 보입니다. 가시적 UI 없이 바로 실행됩니다. 기기가 제스처를 요구하면 화면을 한 번 탭해 주세요.
...
- 바닥이 인식되면 자동 배치합니다. 화면을 탭하면 위치를 다시 잡습니다.
- 히트테스트가 오래 실패하면 시야 정면 1.2m 앞에 자동 배치합니다. AR 미지원이면 간단한 메시지와 함께 일반 iframe 뷰로 대체됩니다.
